...
function damage(item, infusion, stats, upgrade) {
    ...
    let extras = bases.map((amount, ty) => {
        ...
        let scalings = itemInfusion.scaling.map((itemScaling) => {
            return (
                itemScaling * infusion.scaling[ty] + itemScaling * infusion.scaling[ty] * infusion.growth[ty] * upgrade
            )
        })
...
function modifiers(calc, stats, masks) {
    return Object.keys(stats).map((statId, ty) => {
        ...
        let capIndex = calc.softcaps[ty].findIndex((cap) => cap >= stats[statId]) - 1
        ...
        if (Math.sign(adjust) != -1) {
            return growth + growthDelta * ((stats[statId] - cap) / capDelta) ** adjust
        } else {
            return growth + growthDelta * (1 - (1 - (stats[statId] - cap) / capDelta) ** Math.abs(adjust))
        }
    })
}
